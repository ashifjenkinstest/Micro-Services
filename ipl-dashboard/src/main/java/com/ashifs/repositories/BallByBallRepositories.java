...
public interface BallByBallRepositories extends JpaRepository<BallByBall, Long> {
        ...
        @Query(value = "select new  com.ashifs.model.BowlerAttrs(b.bowler,b.matchId, SUM(b.isWicket))  FROM com.ashifs.model.BallByBall as b where b.isWicket = 1 AND ((b.dismissalKind!='RUN OUT') OR (b.dismissalKind='RUN OUT' AND b.fielder = bowler)) AND b.bowlingTeam!='NA' AND b.bowler = :player GROUP BY b.bowler,b.matchId order by SUM(b.isWicket) desc ")
        List<BowlerAttrs> findMaxWicketsAndMatchIdofPlayer(String player);

        @Query("select new  com.ashifs.model.PlayerAndAttribute(b.bowler, sum(b.totalRuns))  FROM com.ashifs.model.BallByBall as b  where b.bowler = :player and b.extrasType!='LEGBYES' and b.matchId=:matchId ")
        PlayerAndAttribute findTotalRunsInHighestWicketMatchofPlayer(String player, Long matchId);

        @Query("select new  com.ashifs.model.BestBowlingFigure(b.bowler ,sum(b.isWicket) ,sum(b.totalRuns))  "
        ...
        @Query(value = " select new com.ashifs.model.PlayerAndAttribute(b.batsman,sum(b.batsmanRuns)) "
